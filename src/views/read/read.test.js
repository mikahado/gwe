  ...
  describe(`s-${material.sessionInfo.sessionId}:p-${material.sessionInfo.partNo} | ${contentId}`, () => {
    it("renders", () => {
      act(() => {
        ReactDOM.render(
          <Router>
            <Read content={content} language={language} />
          </Router>,
          container
        );
      });
    ...
    describe("Congrats Window", () => {
      it("opens after turning last page", () => {
        ...
        for (let i = 0; i <= content.endPage; i++) {
          clickNextPage();
        }
...
function testReadBookMaterial(material) {
  ...
  describe("Next Page Button", () => {
    test("changes page image", () => {
      ...
      expect(img.src).toBe(
        `http://localhost/assets/${content.format}/${content.contentId}/pages/${
          content.startPage + 1
        }.jpg`
      );
    });
  ...
  describe("Page Input", () => {
    it("changes page with number input", () => {
      ...
      for (let i = 1; i < 10; i++) {
        ...
        expect(pageImg.src).toBe(
          `http://localhost/assets/${content.format}/${content.contentId}/pages/${i}.jpg`
        );
      }
    });
  });
  ...
  describe("Spanish Translation", () => {
    if (translationsSpa.hasOwnProperty(content.contentId)) {
      ...
      it('shows "Spanish Coming Soon" message', () => {
        ...
        for (let i = 0; i < pageData.length; i++) {
          const actualText =
            document.getElementById("translation").firstChild.textContent;
